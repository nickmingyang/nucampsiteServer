        ...
        if (favorites) {
          req.body.forEach((campsite) => {
            if (favorites.campsites.indexOf(campsite._id) === -1) {
              ...
              console.log(
                `Campsite ${campsite._id} is already favorited by User ${req.user._id}`
              );
            }
          });
          favorites
  ...
  .delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Favorite.findOne({ user: req.user._id })
      .then((favorites) => {
        if (
          favorites &&
          favorites.campsites.indexOf(req.params.campsiteId) !== -1
        ) {
          ...
          err = new Error(
            `User ${req.user._id} has not favorited any campsites.`
          );
          err.status = 404;
          next(err);
        } else if (favorites.campsites.indexOf(req.params.campsiteId) === -1) {
          err = new Error(
            `User ${req.user._id} has not favorited campsite ${req.params.campsiteId}.`
          );
          err.status = 404;
          next(err);
        }
